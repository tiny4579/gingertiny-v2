...
{
	writel_relaxed(data, host->base + reg);
		  ...
		  unsigned int remain, u32 status)
{
	...
	if ( (remain < 4) ) {
		...
		writel_relaxed(val, base + MMCIFIFO);
		// check the data end
		do {
			...
			void __iomem *base)
{
	...
	if (host->curr.got_dataend && !host->dma.busy) {
		 ...
		 */
		if (readl_relaxed(base + MMCISTATUS) & MCI_RXDATAAVLBL) {
			if (readl_relaxed(base + MMCIMASK1) & (MCI_RXDATAAVLBLMASK
...
#ifdef CONFIG_MMC_PUSHPULL_INIT
				if (host->plat->push_pull_enabled) {
					pr_info("%s: Turn on wifi, reduce duration to 50\n", mmc_hostname(host->mmc));
					duration = 50;
				}
#endif
			}
